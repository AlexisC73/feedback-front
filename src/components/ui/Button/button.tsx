...
export function Button ({children, type = "primary", fullWidth = false, isLoading = false}: {children?: ReactNode, type?: ButtonType, fullWidth?: boolean, isLoading?: boolean}) {
  ...
  return (
    <div className={`pt-3.25 pb-2.75 rounded-2.5 font-bold text-3.5 flex items-center justify-center ${isLoading ? "bg-opacity-50" : ""} ${fullWidth ? "w-full" : "px-2 md:px-5"} ${styles[type]}`}>{isLoading ? <LoadingIcon className="text-5.25" /> : children}</div>
  )
}
