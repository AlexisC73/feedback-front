...
export function Button ({children, type = "primary"}: {children?: ReactNode, type?: ButtonType}) {
  ...
  return (
    <div className={`pt-3.25 pb-2.75 rounded-2.5 font-bold text-3.5 w-full ${styles[type]}`}>{children}</div>
  )
}
